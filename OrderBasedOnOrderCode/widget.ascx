...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        if (!$("link[href='" + $.url('/Content/CustomWidget/dataTables.bootstrap.css') + "']").length) $('<link href="' + $.url('/Content/CustomWidget/dataTables.bootstrap.css') + '" rel="stylesheet" type="text/css"">').appendTo("head");
                ...
                FBAPI.FindExtendedProperty("CustomWidgetData_" + FBUserId + "_" + UserWidgetId,
                        function (msg) {
                            if (msg != null && msg.length > 0) {
                                ...
                                var getDataURL = $.url("/Widgets/OrderBasedOnOrderCode/GetData.ashx?Method=loadData&ProjectID=" + savedChartJSON.ProjectId + "&GroupByFieldID=" + savedChartJSON.GroupByFieldId + "&Search=" + savedChartJSON.FieldValue + "&SearchFrom=" + savedChartJSON.FromDateValue + "&SearchTo=" + savedChartJSON.ToDateValue + "&FieldType=" + savedChartJSON.FieldType);
